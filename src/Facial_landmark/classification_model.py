...
class Classificationmodel:
    ...
    def RFC_mod(self,imgpath,modpath='org'):
        """
        @brief: Use Random Forests to get BMI value
        @param: 
            imgpath: The path of the image that needs to be measured
            modpath: The path of random forests model.
        @return:
    ...
    def GNB_mod(self,imgpath,modpath='org'):
        rd = RD.create_readdata()
        img_x = rd.get_image_x(imgpath)
        if len(img_x) != 0:
            x,y = self.get_data()
    ...
    def SVC_mod(self,imgpath,modpath='org'):
        rd = RD.create_readdata()
        img_x = rd.get_image_x(imgpath)
        if len(img_x) != 0:
            x,y = self.get_data()
    ...
    def MLPC_mod(self,imgpath,modpath='org'):
        rd = RD.create_readdata()
        img_x = rd.get_image_x(imgpath)
        if len(img_x) != 0:
            x,y = self.get_data()
